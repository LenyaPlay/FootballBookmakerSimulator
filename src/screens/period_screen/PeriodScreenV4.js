...
function PeriodScreenV4(props) {
    ...
    return (
        <div className={`${style.periodScreen} ${style.v4}`}>
            ...
            {gameContext.current.matches.map((match, index) => <MatchV4 key={index} match={match} number={index + 1}></MatchV4>)}
            <button className={style.button} onClick={onClickCalculate} style={{ margin: '37px 0px' }} >Calcular o período de jogo</button>
        </div>
    );
}

export default PeriodScreenV4;
